...
void display(){
	...
	while(node != NULL){
		...
		switch (node->type){
			...
			case 3: type = "void"; fprintf(ptr,"%-8s\t\t%-8s\t\t%-8s\t\t%-8d\t\t%-8d\n",node->name, type, "VOID", node->line, node->scope); break; 
...
int lookup(char* name, int scope, int type, int line, int add){ // declarations only
	if(!add)
	{
